...
- (void)configCustomNaviBar {
    ...
    _indexLabel.adjustsFontSizeToFitWidth = YES;
    {
        ...
        _indexLabel.font = manrope ?: [UIFont systemFontOfSize:12 weight:UIFontWeightMedium];
    }
...
- (void)select:(UIButton *)selectButton refreshCount:(BOOL)refreshCount {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            ...
            if (_tzImagePickerVc.singleSelectionMode && _tzImagePickerVc.selectedModels.count > 0) {
                // 清除所有已选择的照片
                ...
                for (TZAssetModel *selectedModel in selectedModels) {
                    ...
                    [_tzImagePickerVc removeSelectedModel:selectedModel];
                    [self setAsset:selectedModel.asset isSelect:NO];
                }
                // 清除photos数组
                if (self.photos) {
            ...
            if (model.type == TZAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
                // 选择视频时的特殊处理
                [self handleVideoSelection:model tzImagePickerVc:_tzImagePickerVc];
            } else if (model.type == TZAssetModelMediaTypePhoto || model.type == TZAssetModelMediaTypePhotoGif) {
                // 检查是否已选择视频
                BOOL hasSelectedVideo = NO;
                for (TZAssetModel *selectedModel in _tzImagePickerVc.selectedModels) {
                ...
                if (hasSelectedVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
                    // 已选择视频时选择照片的特殊处理
                    [self handlePhotoSelectionWhenVideoSelected:model tzImagePickerVc:_tzImagePickerVc];
                } else {
                    // 正常选择逻辑
