...
- (void)setType:(TZAssetCellType)type {
    ...
    if (type == TZAssetCellTypePhoto || type == TZAssetCellTypeLivePhoto || (type == TZAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
        ...
        // Hide time length label for photo cells
        self.timeLength.hidden = YES;
    } else if (type == TZAssetCellTypeVideo) {
        // For video cells, show selection controls but hide bottom view
        _selectImageView.hidden = NO;
        _selectPhotoButton.hidden = NO;
        _bottomView.hidden = YES;
    } else { // Gif
    ...
    if (type == TZAssetCellTypeVideo) {
        // Hide bottom view and video icon for video cells
        self.bottomView.hidden = YES;
        self.videoImgView.hidden = YES;
        
        // Show time length label directly on the cell
        ...
        // Hide time length label for all other types (photos, live photos, etc.)
        self.timeLength.hidden = YES;
    }
}
...
- (UILabel *)timeLength {
    if (_timeLength == nil) {
        ...
        // Set font weight to 500 and size to 9px as requested
        if (@available(iOS 8.2, *)) {
            timeLength.font = [UIFont systemFontOfSize:9 weight:UIFontWeightMedium];
        } else {
            timeLength.font = [UIFont boldSystemFontOfSize:9];
        }
        timeLength.textColor = [UIColor whiteColor];
        timeLength.textAlignment = NSTextAlignmentRight;
        // Add directly to contentView instead of bottomView
        [self.contentView addSubview:timeLength];
...
- (UILabel *)indexLabel {
    if (_indexLabel == nil) {
        ...
        UIFont *manrope = [UIFont fontWithName:@"Manrope-Medium" size:12];
        indexLabel.font = manrope ?: [UIFont systemFontOfSize:12 weight:UIFontWeightMedium];
...
- (void)layoutSubviews {
    ...
    // Position time length label at bottom right with 8px margins
    if (self.model.type == TZAssetCellTypeVideo) {
        // For video cells, position at bottom right with 8px margins
        CGFloat labelWidth = 60; // Approximate width for time labels
        CGFloat labelHeight = 12; // Height for 9px font
        _timeLength.frame = CGRectMake(self.tz_width - labelWidth - 8, self.tz_height - labelHeight - 8, labelWidth, labelHeight);
    } else {
        // For GIF cells, keep original positioning
...
- (void)layoutSubviews {
    ...
    // 相册照片尺寸70px，距离左边16px
    self.posterImageView.frame = CGRectMake(16, 0, 70, 70);
    
    // 相册标题和数量描述垂直排列，距离照片右侧10px
    CGFloat textX = 16 + 70 + 10; // 左边距16 + 照片宽度70 + 间距10
    CGFloat textWidth = self.tz_width - textX - 16; // 右边距16
    
    // 计算两个label的总高度和间距
    NSInteger titleHeight = ceil(self.titleLabel.font.lineHeight);
    NSInteger countHeight = ceil(self.countLabel.font.lineHeight);
    CGFloat totalHeight = titleHeight + 4 + countHeight; // 标题高度 + 间距4px + 数量高度
    
    // 计算起始Y位置，使两个label整体在cell中垂直居中
    CGFloat startY = (self.tz_height - totalHeight) / 2;
    
    // 相册标题位置
    self.titleLabel.frame = CGRectMake(textX, startY, textWidth, titleHeight);
    
    // 图片数量描述位置（在标题下方4px）
    CGFloat countY = startY + titleHeight + 4;
    self.countLabel.frame = CGRectMake(textX, countY, textWidth, countHeight);
