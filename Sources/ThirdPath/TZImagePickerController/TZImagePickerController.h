...
/// Default tab index when photo picker appears, 0=All, 1=Video, 2=Photos
/// 照片选择器出现时的默认tab索引，0=全部，1=视频，2=照片
...
/// Default is NO, if set YES, only one photo can be selected at a time, but still uses multi-selection logic
/// 默认是NO，如果设置为YES，每次只能选择一张照片，但仍然走多选逻辑
@property (assign, nonatomic) BOOL singleSelectionMode;
