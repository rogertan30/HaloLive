...
- (void)configNavigationBarAppearance {
    if (@available(iOS 13.0, *)) {
        ...
        // For iOS 12 and below, remove the shadow image
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<TZImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:3 delegate:delegate pushPhotoPickerVc:YES];
}
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        self.defaultTabIndex = 0; // Default to "All" tab
...
- (void)configDefaultSetting {
    ...
    self.iconThemeColor = [UIColor colorWithRed:0x70 / 255.0 green:0x3E / 255.0 blue:0xFF / 255.0 alpha:1.0];
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        ...
        [[TZImageManager manager] getCameraRollAlbumWithFetchAssets:NO completion:^(TZAlbumModel *model) {
            ...
            // Set default tab after view appears
            dispatch_async(dispatch_get_main_queue(), ^{
                [photoPickerVc switchToTab:self.defaultTabIndex];
            });
        }];
    }
}
...
- (UIImage *)createImageWithColor:(UIColor *)color size:(CGSize)size radius:(CGFloat)radius {
    ...
    // Draw 1px white border around the circle
    CGContextSetStrokeColorWithColor(context, [UIColor whiteColor].CGColor);
    CGContextSetLineWidth(context, 1.0);
    UIBezierPath *strokePath = [UIBezierPath bezierPathWithRoundedRect:CGRectInset(rect, 0.5, 0.5) cornerRadius:radius];
...
- (void)viewDidLoad {
    ...
    [closeButton addTarget:imagePickerVc action:@selector(cancelButtonClick) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *closeItem = [[UIBarButtonItem alloc] initWithCustomView:closeButton];
    self.navigationItem.leftBarButtonItem = closeItem;
    
    // Add notification observer for album switching
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleAlbumSwitchedNotification:) name:@"TZ_ALBUM_SWITCHED_NOTIFICATION" object:nil];
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (imagePickerVc.allowPickingImage) {
        titleText = [NSBundle tz_localizedStringForKey:@"Photos"];
    } else if (imagePickerVc.allowPickingVideo) {
        titleText = [NSBundle tz_localizedStringForKey:@"Videos"];
    }
...
- (void)animateArrowRotation {
    ...
    CGFloat targetRotation = self.isAlbumPickerVisible ? M_PI : 0; // 180 degrees when showing, 0 when hiding
...
- (void)loadAlbumDataForSlideDownPicker {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [[TZImageManager manager] getAllAlbumsWithFetchAssets:YES completion:^(NSArray<TZAlbumModel *> *models) {
            dispatch_async(dispatch_get_main_queue(), ^{
                // Use the same album data as the main table view
                if (self->_albumArr) {
...
- (void)handleAlbumSwitchedNotification:(NSNotification *)notification {
    // Update album picker when album is switched
    [self configTableView];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // If this is the slide-down album picker, dismiss it first
    if (tableView == self.albumPickerTableView) {
        [self dismissAlbumPicker];
        // Push photo picker after a short delay to allow dismissal animation to complete
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self.navigationController pushViewController:photoPickerVc animated:YES];
            // Set default tab after view appears
            dispatch_async(dispatch_get_main_queue(), ^{
                [photoPickerVc switchToTab:imagePickerVc.defaultTabIndex];
            });
        });
    } else {
        // Regular table view behavior
        [self.navigationController pushViewController:photoPickerVc animated:YES];
        // Set default tab after view appears
        dispatch_async(dispatch_get_main_queue(), ^{
            [photoPickerVc switchToTab:imagePickerVc.defaultTabIndex];
...
+ (BOOL)isAssetNotSelectable:(TZAssetModel *)model tzImagePickerVc:(TZImagePickerController *)tzImagePickerVc {
    ...
    if (tzImagePickerVc.selectedModels && tzImagePickerVc.selectedModels.count > 0 && !tzImagePickerVc.allowPickingMultipleVideo) {
        // 检查已选择的资源中是否有图片
        BOOL hasSelectedImage = NO;
        for (TZAssetModel *selectedModel in tzImagePickerVc.selectedModels) {
            if (selectedModel.asset.mediaType == PHAssetMediaTypeImage) {
                hasSelectedImage = YES;
                break;
            }
        }
        // 如果已选择图片且当前资源是视频，则置灰视频
        if (hasSelectedImage && model.asset.mediaType == PHAssetMediaTypeVideo) {
            notSelectable = true;
        }
        // 移除选择视频时图片置灰的逻辑
    }
